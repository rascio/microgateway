...
abstract class AbstractHandler implements HandlerFunction<ServerResponse> {
    ...
    protected <T extends Request<?, ?, ?, ?>> Envelope<T> envelope(ServerRequest request, T message) {
        final HttpHeaders headers = request.headers().asHttpHeaders();
        return request.principal()
            .map(principal -> new Envelope<>(headers, message, principal))
            .defaultIfEmpty(new Envelope<>(headers, message, null))
            .block();
    }
